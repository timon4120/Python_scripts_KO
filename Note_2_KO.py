...
class Ising():
    def __init__(self,n,J,T,B,steps,dens):
        self.n = n
        self.J = J
        self.T = T
        self.B = B
    ...
    def Get_E(self,x,y,mark = 1):
        ...
        neighbourhood = -self.J * np.sum(neig) * mtx[x,y]
        field = -self.B * np.sum(mtx)
    ...
    def Simulation(self,step_tit,anim_tit):
        for step in rich.progress.track(range(self.steps)):
            for i in range(self.n * self.n):
                ...
                if delta < 0: 
                    self.matrix[x,y] *= -1
                elif rd.uniform(0,1) < np.exp(-1/self.T * delta): 
                    self.matrix[x,y] *= -1
    ...
    def animate2(self,i):
        ...
        plt.title(f'Model Isinga \n N: {self.n} | J: {self.J} | T: {self.T} | B: {self.B} | Dens: {self.dens} | t: {i} | M: {round(self.M[i],2)} \n Kamil Orzechowski')
...
def main():
    ...
    model = Ising(int(args.n),float(args.j),float(args.T),float(args.B),int(args.s),float(args.d))
    print("Start")
